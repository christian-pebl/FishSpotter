...
| `src/data/species-traits.json` | Trait catalogue for the IdGuideWizard (body shape, size, markings, behaviour, habitat, plus the prose `fieldNote`). Read at request time by the wizard's narrowing engine in `src/lib/idguide/narrow.ts`. **34 species as of 1 Jun 2026** (26 fish, 1 flatfish, 1 scooter, 6 crabs). Every entry carries `shapeClass` + `movement` (Workstream A). Crab entries also carry the optional crab-only traits `carapaceTexture` + `crabFeatures` (Workstream C); fish entries omit them. See `implementation/2026-06-01/`. |
