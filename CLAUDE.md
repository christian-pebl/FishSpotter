...
| `src/lib/site-label.ts` | **How a location is printed, everywhere (3 Sep 2026).** `Snippet.site` is the KEY ("Ramsey Sound, Pembrokeshire, Wales, UK": the archive filter, the URL param, the sync's value) but people know the places by their seaweed and shellfish farms, so every surface runs a site through `siteLabel()` and leads with the farm: "Câr-y-Môr · Ramsey Sound, Pembrokeshire, Wales, UK" (`shortSiteLabel()` gives "Câr-y-Môr · Ramsey Sound" for lists and tight spaces). The farm comes from `siteNames` on each farm in `src/data/seaweed-farms.json`, next to `deploymentNames`, so a new or renamed site is a one-file change; `site-label.test.ts` proves every farm with clips names a site and no site is claimed twice. Non-farm sites print as they are. Never print a raw `site` on a user-facing surface. |
| `src/app/farms/page.tsx` | **"About the Project" tab (9 Sep 2026).** The `/farms` route, renamed from "The seaweed farms" in the nav and the H1, URL unchanged. Leads with the programme the farms sit inside (*Unlocking the Power of Seaweed*: the National Lottery Climate Action Fund project WWF-UK leads with Câr-y-Môr, SAMS and PEBL) and with what PEBL is trying to find out (biodiversity trends at seaweed and shellfish farms, farm-versus-control cameras, the 2022-23 Câr-y-Môr baseline), then the unchanged six farm cards with their live clip counts. **Terminology invariant: collective copy says "seaweed and shellfish farms", never "seaweed farms"** — but never rewrite a farmer's direct quote, a descriptor of one seaweed-only site, or WWF's own published project aim to fit it. |
