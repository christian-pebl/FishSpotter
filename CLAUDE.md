...
| `src/app/feed/browse/page.tsx` | Video archive grid page. Each card opens the live feed at that clip, carrying its site/q/sort as search params. |
| `src/app/feed/[id]/page.tsx` | One clip, opened IN the live feed: same `FeedPlayer`, ordered so the tapped clip is first and the rest of the archive follows behind it, wrapping past the end. Replaced a bespoke `SnippetPlayer` (letterboxed `<video>` + details card + type-the-name box, deleted 30 Aug 2026) that made the archive lead somewhere strictly worse than the app's main surface and dead-ended a spotter who answered there. The URL is unchanged, so shared links, the sitemap and the profile's identification list still land correctly. |
| `src/lib/archive-query.ts` | The archive's filter/sort contract (`archiveWhere` / `archiveOrderBy` / `archiveFilterQuery` / `rotateToClip`), shared by the grid and the feed it opens into. It exists because "the next clip" is only truthful while both build the SAME query from the same params; two copies would drift silently, serving a different next clip than the grid showed with nothing failing. `archiveOrderBy` always tie-breaks on `id` for the same reason (`site` leaves Postgres free to return a deployment's bucket in a different order per query). |
