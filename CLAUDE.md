...
- **[docs/runbooks/transactional-email.md](docs/runbooks/transactional-email.md)**: how verification / reset emails are sent, and the order to check things in when a spotter says one never arrived (health, `/admin/email`, SendGrid, DNS), plus verifying someone by hand.
...
| `src/lib/site-label.ts` | **How a location is printed, everywhere (3 Sep 2026).** `Snippet.site` is the KEY ("Ramsey Sound, Pembrokeshire, Wales, UK": the archive filter, the URL param, the sync's value) but people know the places by their seaweed farms, so every surface runs a site through `siteLabel()` and leads with the farm: "Câr-y-Môr · Ramsey Sound, Pembrokeshire, Wales, UK" (`shortSiteLabel()` gives "Câr-y-Môr · Ramsey Sound" for lists and tight spaces). The farm comes from `siteNames` on each farm in `src/data/seaweed-farms.json`, next to `deploymentNames`, so a new or renamed site is a one-file change; `site-label.test.ts` proves every farm with clips names a site and no site is claimed twice. Non-farm sites print as they are. Never print a raw `site` on a user-facing surface. |
...
| `src/components/FeedPlayer.tsx` | IntersectionObserver scroll container; sets activeIndex. **Windowed (7 Sep 2026):** every clip keeps its full-height section (scroll positions, snap points and keyboard/observer navigation are unchanged) but a `FeedCard` is MOUNTED only within `RENDER_WINDOW` (3) of the active card; the rest are the dark placeholder a card outside the still window already looked like. Mounting all 139 cost a 4x-throttled phone 932 ms of blocking time to hydrate and put 139 `<video>` elements in the DOM, and every scroll step re-rendered every card. Consequence handled here: a guest guess whose card is no longer mounted has nothing to carry it in on signup, so `drainGuestAnswers` posts those (skipping the mounted ids, which drain their own). |
| `src/lib/split-screen.ts` | **The split-screen contract** (docked panel beside the clip on a wide viewport, bottom sheet under it on a phone): sizing constants, the stored panel size, the `fs-gate` frame bus with a cached snapshot, and the `--gate-left` / `--gate-bottom` / `--fs-panel-*` custom properties on the DOCUMENT element that the clip frame and portaled overlays read in pure CSS. **Everything it returns is right on a component's FIRST render** (7 Sep 2026): `useDocked`, `useStoredSplitSize` and the frame hooks are `useSyncExternalStore` stores, so a panel opened by a tap renders in its final orientation at the viewer's stored size, where it used to paint two or three frames as a phone sheet (clip squashed into the top half) before jumping to the docked layout, on every open. The frame is published from a layout effect, so the clip resizes in the same frame the panel appears. Subscribe with the boolean selectors (`useSplitOpen`, `useSplitDocked`) rather than `useSplitFrame`: the frame changes on every pixel of a resize drag. Feed cards do not subscribe at all; they inherit the root custom properties. |
| `src/lib/useMediaQuery.ts` | `useMediaQuery(query)`: viewport questions answered on the first render via `useSyncExternalStore`, one shared `MediaQueryList` per query. Server and hydration render `false` (matching the HTML); anything mounted after hydration reads the real match immediately. **Use this, never `useState(false)` + a `matchMedia` effect**, which is a painted frame of the wrong layout for every component opened by a tap. |
| `src/lib/guestAnswers.ts` | The guest guess queue (localStorage, 24h, 50 max) that "play before the wall" fills, plus `drainGuestAnswers(userId, mountedIds)`, which posts the entries whose cards are not mounted once the viewer signs in (entries are taken out of the queue before each request; network/5xx failures are put back, 4xx dropped). Mounted cards still drain their own entry in `useCreatureQuiz` so the reveal lands in front of the viewer. |
| `scripts/bench/*.cjs` | **Load benchmarks (7 Sep 2026).** `npm run bench:load -- <url> <label> 3` (route timings, payload, DOM size, long tasks at 1280x800), `npm run bench:phone -- <url> <label> 3` (`/feed` at 390x844 with 4x CPU throttling: time to tappable, tap-to-panel, blocking time) and `npm run bench:split -- <url> <label>` (frame-by-frame record of the identify panel opening, the only measurement that can see a first-paint flash). Results and the method live in `implementation/2026-09-07/load-benchmark.md`; **measure before and after any change to the feed, the split or media, and append a dated section there.** Compare only runs from the same machine against the same target; a dev server is not comparable to a production build. |
| `src/lib/thumbnail-src.ts` | `thumbnailSrc(url)`: routes a snippet still through Next's image optimizer (`/_next/image?...&w=1080`) so the feed's poster, sharp still and blurred backdrop share one ~10 KB WebP instead of the 170 to 560 KB archive JPEG (measured 7 Sep 2026: 170,713 to 10,592 bytes). `next.config.mjs` sets `images.minimumCacheTTL` to 30 days because every image URL is immutable (`?v=` on stills, content hashes on species WebPs) while the upstream objects only say `max-age=3600`. |
...
| `src/app/feed/browse/page.tsx` | Video archive grid page. Filter row is **Species → Location → Sort** (30 Aug 2026); the free-text "Search species, site, deployment" box is gone, because the only species text it could match was `staffAnswer`, which is shape words. Each card opens the live feed AT that clip, carrying the same filter and sort (never `page`, the feed runs past the grid's page end). |
| `src/app/feed/[id]/page.tsx` | One clip, opened IN the live feed: same `FeedPlayer`, ordered so the tapped clip is first and the rest of the archive follows behind it, wrapping past the end. The archive's THIRD entry into the feed (after `/feed` and the grid's "Launch feed" button) and the only one where ORDER matters as much as the set, so the set comes from `snippet-filter.ts` like the other two and the order from `archive-query.ts`. Replaced a bespoke `SnippetPlayer` (letterboxed `<video>` + details card + type-the-name box, deleted 30 Aug 2026) that made the archive lead somewhere strictly worse than the app's main surface and dead-ended a spotter who answered there. URL unchanged, so shared links, the sitemap and the profile's identification list still land correctly. |
| `src/lib/archive-query.ts` | The archive's ORDER (`archiveOrderBy`, `parseArchiveSort`) and the rotation that turns it into a feed (`rotateToClip`). Deliberately the other half of `snippet-filter.ts`, which owns the SET: order and window are archive-only, except on `/feed/[id]`, where "the next clip is the next one in the grid" is only truthful while the grid and that feed sort identically. `archiveOrderBy` always tie-breaks on `id`, because `site` leaves Postgres free to return a deployment's bucket in a different order per query. `rotateToClip` WRAPS rather than truncating, so a clip opened at the bottom of the archive still has a next. |
...
| `src/lib/age.ts` | **Children's rules (16 Sep 2026).** The one place that says what each self-declared age band may do: public naming (`canBePubliclyNamed` + `PUBLICLY_NAMED_WHERE`), comments, optional and streak emails, analytics, AI chat, own email, and whether a prize needs a parent. Bands are `under_13` / `13_17` / `18_plus`; null means not asked yet and is treated as possibly a child. Every surface that names, emails, tracks or rewards a spotter must ask this module, never re-derive the rule. Why and the legal mapping: `docs/compliance/children.md`. |
| `src/lib/parental-consent.ts` | **Parental consent, COPPA "email plus".** One `ParentalConsent` row per child and purpose (`account` for an under-13 to keep an account, `prize` for any under-18). Request, grant and decline; manage links and single-use child sign-in links (`ParentAccessToken`, hashed); the day-later confirmation (`findDueConfirmations`); and the retention purge (`purgeChildData`), both run by `/api/cron/child-data-retention` at 05:00 UTC. Client code imports the shared vocabulary from `parental-consent-shared.ts` (this module pulls in node:crypto). Pages: `/parent`, `/parent/consent/[token]`, `/parent/manage/[token]`; APIs under `/api/parent/*`. |
| `src/components/age/AgeCheck.tsx` | The one-time age question for any signed-in account with no band (mounted in the root layout with `AskParentDialog`). Answerable, not dismissable; waits on the policy and parent pages. Guests are asked in `GuestGate` before they pick a name; signup asks on the form and sends under-13s to nickname play. Open it from anywhere with `requestAgeCheck()` (`src/lib/age-events.ts`). |
| `src/lib/admin.ts` | S9-T1 admin gate: `isAdminUser()` requires BOTH the `@pebl-cic.co.uk` suffix AND a verified email (`emailVerified` non-null), domain alone is not enough because guest-claim (`POST /api/guest/claim`) can write an unverified, arbitrary email into `User.email` (fixed 2026-07-16 Critical audit finding, was a guest->admin escalation). `getAdminSession()`/`requireAdminSession()` do the lookup and redirect non-admins to `/`. Used by the `/admin` layout + the diagnostic-mark server actions + the private per-user answers view on `/u/[id]`. Setting a password from an emailed link (`POST /api/auth/reset`) also confirms the address since 16 Sep 2026, **but never on the admin domain** (`src/lib/admin-email.ts`), so this gate still needs the explicit verification click. |
| `src/lib/email/outcome.ts` | **What happened to an email, and what to tell a person (8 Sep 2026).** `sendEmail()` (`send.ts`) never throws and returns `{ ok, skipped?, messageId?, error? }`; **`ok` alone does not mean "sent"**: an unconfigured provider answers `ok: true, skipped: true` so a caller's own transaction is never rolled back. `sendOutcome()` / `wasSent()` are the honest reading, and every surface that says "check your inbox" uses them. Exists because a new spotter pressed "resend verification" for days while `POST /api/auth/verify-request` answered 200 for messages that never left (SendGrid not configured, or refusing the sender, were both a console line and nothing else). Now: resend and forgot-password answer **503** with `EMAIL_UNAVAILABLE_MESSAGE` when nothing can be sent, the signup email is **awaited** in `authorize()` (an un-awaited promise on a serverless runtime can be frozen with the function, which is how first emails went missing), a resend no longer retires earlier links, `/api/health` reports `email: configured | unconfigured`, and `VerificationHelp.tsx` puts "check spam, or email hello@pebl-cic.co.uk and we will verify you by hand" under every resend button. Runbook: `docs/runbooks/transactional-email.md`. |
| `src/app/admin/email/page.tsx` | **Email delivery diagnostics.** Three checks, cheapest first, then a fix (section 4, the catch-up send, see `verification-backlog.ts` below): is the sender configured (env var names, never values); are verification emails getting through (`src/lib/email/verification-stats.ts`, pure: `VerificationToken` rows minted vs. consumed by a real click, where a click is a consumed token whose owner's `emailVerified` was stamped in the same moment, so tokens the old resend retired never count); and a **test send** to the admin's own address that quotes Resend verbatim on a refusal (a 403 saying the domain is not verified is the whole diagnosis, and it used to live only in Vercel's function logs). |
| `src/lib/prize-requirements.ts` | **The prize claim rules as a checklist (16 Sep 2026).** `prizeClaimStatus()` returns the Pebble target, the account step (a guest must save an email first, anyone else must confirm theirs), five separate spotting days and a 14-day spread, each with progress text, computed with the SAME constants and the same `measureActivity()` as `isPrizeEligible` in `trust.ts`, so the card can never promise a claim the route refuses (a test sweeps inputs to prove the two agree). The trust score is never listed; `trustPending` marks the case where it is the only thing left, and the card then says "almost there" without naming it. Built because a production pull found the unseen rules were what blocked everyone: 1 of 95 public spotters had five spotting days, and 38 had never confirmed an email, including five of the top six. `PrizeCard` shows the list from the first Pebble (it used to surface one generic reason only after 2,000), with a "Send me the link" verification action and an "Add my email" action that opens `GuestSavePrompt` via `GUEST_SAVE_REQUEST_EVENT` (`src/lib/guest.ts`). `prizeRulesSummary()` is the one-sentence version for signed-out visitors. **Import it only from server code**: it pulls `trust.ts`, which pulls the catalogue; the client card takes the result as props. |
| `src/lib/email/verification-backlog.ts` | **The catch-up send (16 Sep 2026)**, section 4 of `/admin/email`. Every non-guest account with no confirmed address gets ONE fresh link: a set-a-password link (`sendAccountSetupEmail` in `dispatch.ts`, 3-day TTL) when the account has no password, since using it also confirms the address, otherwise a verification link (7-day TTL). The admin domain always gets the verification link, because setting a password never confirms an address there. 24-hour cooldown against ANY recent link, re-decided server-side per account in `sendCatchUpEmail` (the page loops one account per request, 600 ms apart, and stops after two refusals in a row). Previews go to the admin with a dead token, so they can never set a password or confirm anything. Found on 16 Sep: 41 such accounts, 32 of them guests who saved their progress and never received a set-a-password link, so they could not sign back in. |
| `src/app/about/page.tsx` | **The story of the project (8 Sep 2026)**, asked for by a new spotter who could not find one: who films the clips, the National Lottery Climate Action Fund project with WWF, the six host farms, and what identifications are used for (crowd consensus, anonymised species records for PEBL's monitoring and UK marine biodiversity science, engagement figures to the funder only with analytics consent, never sold). Every claim is one the landing page, farms page, privacy policy or LIA already makes; nothing about partners or data use is asserted beyond those. Linked from the side menu ("About the project"), the landing page, the farms page and the account page, because a signed-in spotter never sees the landing page (middleware sends them to `/feed`). |
...
- **Client state that the server cannot know (viewport, localStorage) must be right on the FIRST render of anything mounted after hydration.** Use `useMediaQuery` / the `useSyncExternalStore` stores in `src/lib/split-screen.ts`, not `useState(placeholder)` plus a mount effect. That pattern is hydration-safe and is also a painted frame of the wrong layout on every open (the 7 Sep 2026 sheet-then-dock flash). A regression guard that can see it has to record frames (`tests/e2e/split-orientation.spec.ts`); a settled-state assertion passes throughout the bug's life.
...
**Cache headers (7 Sep 2026):** every live clip and still on Supabase now serves
`Cache-Control: public, max-age=2592000`, put there by `scripts/fix-media-cache-control.ts`
(identical bytes, same URLs, no DB writes). New uploads get it from the driver. If a re-upload
is ever run from a checkout whose `storage.ts` lacks the `cacheControl` option, the objects
silently fall back to `max-age=3600` again; check with an S3 `HeadObject` or a full GET of the
public URL some minutes after the write, never a HEAD (it says `no-cache` regardless) and never
a read straight after the write (the CDN serves its old copy for a while).
...
- `User`: id, email, displayName, name, **`ageBracket`** (`under_13` / `13_17` / `18_plus`, null = not asked; see `src/lib/age.ts`), `ageDeclaredAt`
- `ParentalConsent` / `ParentAccessToken` (16 Sep 2026): a parent's consent for a child's account or prize, and the hashed links parents use; see `src/lib/parental-consent.ts`. Added by `prisma db push`, so run `npm run db:enable-rls` after it.
...
RESEND_API_KEY=...                # transactional email (src/lib/email/client.ts). Resend since 15 Sep 2026: SendGrid's
                                  # free trial had run out and it refused every send for six weeks (runbook section 7)
EMAIL_FROM_ADDRESS=noreply@fishspotter.app  # REQUIRED with the key, or nothing is sent (health says
                                  # email: unconfigured, resend/forgot answer 503); must be on a domain
                                  # verified in Resend (three DNS records in Cloudflare). docs/runbooks/transactional-email.md
EMAIL_FROM_NAME=FishSpotter
EMAIL_REPLY_TO=hello@pebl-cic.co.uk
EMAIL_PREVIEW_CATCHALL=...        # preview deploys only: every email is redirected here
