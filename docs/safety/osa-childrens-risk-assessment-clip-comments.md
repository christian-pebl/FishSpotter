...
**Status:** WORKING DRAFT prepared by engineering, based on the actual
implementation. **Not a substitute for formal legal/compliance sign-off.**
...
- Every comment is visible to any spotter who has answered that clip
  ...
  **Recommended for review:** whether age-segregated visibility is
  proportionate to build, given the service's actual usage pattern (citizen
  science, not a social platform) and the cost of building and maintaining
  reliable age segregation without real age verification.
...
1. Formal review and sign-off by Christian / PEBL's accountable person,
   with explicit attention to §4.2 (age-segregation trade-off) — this is
   the one judgement call in this document that is genuinely Christian's to
   make, not a technical determination.
2. Decide whether age-segregated comment visibility is worth building
   (§4.2) — likely not proportionate given the service's actual profile,
   but the decision should be recorded, not defaulted into.
3. Monitor whether any reports on `/admin/comments` involve a declared-minor
   account, either as reporter or subject, once real usage data exists —
   this is the strongest signal for revisiting this assessment's ratings.
4. Re-run this assessment if age verification is ever added to the
   platform (it would materially change what mitigations are feasible), or
   if a real incident involving a minor occurs.
