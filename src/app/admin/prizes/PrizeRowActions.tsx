...
export function CopyEmailButton({ email }: { email: string }) {
  ...
  return (
    <button
      ...
      className="inline-flex h-11 items-center rounded-full px-3 text-[11px] font-medium text-teal-600 transition hover:bg-teal-50 hover:text-teal-700"
    >
      {copied ? "Copied" : "Copy"}
    </button>
  );
  ...
  return (
    <div className="flex flex-col items-start gap-1">
      <button
        ...
        className={`inline-flex h-11 items-center rounded-full px-4 text-[11px] font-semibold transition disabled:opacity-60 ${
          posted
